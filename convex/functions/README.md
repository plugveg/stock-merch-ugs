...
const relationship = await ctx.db
  .query('userProducts')
  .withIndex('by_owner_and_product', (q) => q.eq('ownerUserId', userId).eq('productId', productId))
  .first()
...
export const getProductsForTransaction = query({
  ...
  handler: async (ctx, args) => {
    ...
    return await getManyVia(ctx.db, 'transactionProducts', 'productId', 'transactionId', args.transactionId)
  },
})
...
export const getTransactionsForProduct = query({
  ...
  handler: async (ctx, args) => {
    ...
    return await getManyVia(ctx.db, 'transactionProducts', 'transactionId', 'productId', args.productId)
  },
})
...
export const getTransactionsForBuyer = query({
  ...
  handler: async (ctx, args) => {
    // Use the getManyVia helper to get transactions via the junction table
    return await getManyVia(ctx.db, 'userBuyerTransactions', 'transactionId', 'buyerUserId', args.buyerUserId)
  },
})
...
export const getTransactionsForSeller = query({
  ...
  handler: async (ctx, args) => {
    // Use the getManyVia helper to get transactions via the junction table
    return await getManyVia(ctx.db, 'userSellerTransactions', 'transactionId', 'sellerUserId', args.sellerUserId)
  },
})
...
export const getCompleteTransaction = query({
  ...
  handler: async (ctx, args) => {
    ...
    const products = await getManyVia(ctx.db, 'transactionProducts', 'productId', 'transactionId', args.transactionId)
    ...
    const buyer = buyerRelation ? await ctx.db.get(buyerRelation.buyerUserId) : null
    ...
    const seller = sellerRelation ? await ctx.db.get(sellerRelation.sellerUserId) : null
...
export const getProductsForTransaction = query({
  ...
  handler: async (ctx, args) => {
    ...
    return await Promise.all(relationships.map((rel) => ctx.db.get(rel.productId)))
  },
})
