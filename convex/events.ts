...
export const addUserToEvent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existingParticipant = await ctx.db
      .query('eventParticipants')
      .withIndex('by_eventId_and_userId', (q) => q.eq('eventId', args.eventId).eq('userId', userToAdd._id))
      .unique()
...
export const removeUserFromEvent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!canRemove) {
      throw new Error('You do not have permission to remove this user from the event.')
    }
    ...
    if (args.userIdToRemove === event.adminId) {
      const otherOrganizers = await ctx.db
        .query('eventParticipants')
        .withIndex('by_eventId_and_role', (q) => q.eq('eventId', args.eventId).eq('role', 'Administrator'))
    ...
    const participantEntry = await ctx.db
      .query('eventParticipants')
      .withIndex('by_eventId_and_userId', (q) => q.eq('eventId', args.eventId).eq('userId', args.userIdToRemove))
      .unique()
...
export const removeProductFromEventSale = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (organizers.length === 0 && event.adminId !== meDoc._id) {
      throw new Error('Only event organizers or the event admin can remove products from the event sale.')
    }
...
export const updateUserRoleInEvent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const participantEntry = await ctx.db
      .query('eventParticipants')
      .withIndex('by_eventId_and_userId', (q) => q.eq('eventId', args.eventId).eq('userId', args.userId))
      .unique()
