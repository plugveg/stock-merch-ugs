...
export default function AdminDashboard() {
  ...
  const updateEventProductStatus = useMutation(api.events.updateEventProductStatus)
  const removeProductFromEventSale = useMutation(api.events.removeProductFromEventSale)
  ...
  const otherUserProducts = useQuery(api.functions.products.listAllProductsByStatus, { status: 'For Event Sale' })
  ...
  const eventDetails = useQuery(api.events.getEventDetails, selectedEventId ? { eventId: selectedEventId } : 'skip')
  const eventAnalytics = useQuery(api.analytics.getEventAnalytics, selectedEventId ? { eventId: selectedEventId } : 'skip')
  ...
  const handleCreateEvent = async (e: FormEvent) => {
    ...
    try {
      ...
      console.error(error)
      alert(`	Erreur lors de la création de l'événement : ${(error as Error).message}`)
    }
  }
  ...
  const handleAddUserToEvent = async (e: FormEvent) => {
    ...
    try {
      ...
      console.error(error)
      alert(`Erreur : l'utilisateur existe déjà ou autre problème : ${(error as Error).message}`)
    }
  }
  ...
  const handleUpdateStatus = async (eventProductId: Id<'eventProducts'>, status: Status) => {
    try {
  ...
  const handleRemoveProduct = async (eventProductId: Id<'eventProducts'>) => {
    if (!confirm("Voulez-vous vraiment supprimer ce produit de la vente lors de l'événement ?")) return
    try {
  ...
  const handleRemoveUser = async (userIdToRemove: Id<'users'>) => {
    ...
    if (!confirm("Etes-vous sûr de vouloir retirer cet utilisateur de l'événement ?")) return
    try {
      await removeUserFromEvent({ eventId: selectedEventId, userIdToRemove })
      alert("Utilisateur retiré de l'événement !")
    } catch (error) {
      console.error(error)
      alert(`Erreur lors de la suppresion de l'utilisateur de l'événement : ${(error as Error).message}`)
    }
  }
    ...
    <div className="min-h-screen bg-linear-to-br from-indigo-100 to-purple-100 flex flex-col">
      <header>
        <NavBar>
          ...
          {/* Texte identifiant */}
          <span className="content-center">Connecté en tant que {userInConvex?.nickname ?? userInConvex?.email}</span>
        </NavBar>
      </header>
      <main className="flex-1 p-4 md:p-6 mb-[80px] md:mb-[56px] mt-[140px] md:mt-[56px] space-y-8">
        <section className="p-6 bg-white rounded-lg shadow">
          <h2 className="text-2xl font-semibold mb-4 text-primary">Créer un nouvel événement</h2>
          <form onSubmit={handleCreateEvent} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            />
            <Button type="submit" className="bg-primary text-white p-2 md:col-span-2">
              Créer l'événement
            </Button>
          </form>
        </section>
        ...
        <section className="p-6 bg-white rounded-lg shadow">
          <h2 className="text-2xl font-semibold mb-4 text-primary">Gérer l'événement</h2>
          <select
          ...
          {selectedEventId && eventDetails && (
            <div className="space-y-6">
              <div>
                <h3 className="text-xl font-semibold mb-2">Événement : {eventDetails.name}</h3>
                <p>{eventDetails.description}</p>
                <p>
                  Time: {new Date(eventDetails.startTime).toLocaleString()} - {new Date(eventDetails.endTime).toLocaleString()}
                </p>
              </div>

              {/* Add User to Event */}
              <form onSubmit={handleAddUserToEvent} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
              ...
              <form onSubmit={handleAddProductToSale} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <select
                  value={productIdToAdd}
                  onChange={(e) => setProductIdToAdd(e.target.value as Id<'products'>)}
                  className="p-2 border rounded"
                  required
                >
                  <option value="" disabled>
              ...
              {/* Event Products List */}
              <div>
                <h4 className="text-lg font-semibold mt-4 mb-2">Produits à vendre durant l'événement :</h4>
                {eventDetails.products.length === 0 && <p>Pas encore de produits ajouter pour cet événement</p>}
                <ul className="space-y-2">
                  {eventDetails.products.map((ep) => (
                    <li key={ep._id} className="p-3 border rounded-md shadow-sm flex justify-between items-center">
                      <div>
                        <span className="font-medium">{ep.productName}</span> - €{ep.salePrice ? ep.salePrice.toFixed(2) : 'N/A'}{' '}
                        (Statut: {ep.status})
                      </div>
                      <div className="space-x-2">
                        ...
                        {ep.status !== 'On Sale' && (
                          <Button
                            onClick={() => handleUpdateStatus(ep._id, 'On Sale')}
                            size={'sm'}
                        ...
                        {ep.status !== 'Reserved' && (
                          <Button
                            onClick={() => handleUpdateStatus(ep._id, 'Reserved')}
                            size={'sm'}
              ...
              <div>
                <h4 className="text-lg font-semibold mt-4 mb-2">
                  Participants ({eventAnalytics ? eventAnalytics.participantCount : 0}):
                </h4>
                <ul className="space-y-2">
                  {eventDetails.participants.map((p) => (
                    <li key={p.userId} className="p-3 border rounded-md shadow-sm flex justify-between items-center">
                      <div className="flex items-center space-x-4">
                        ...
                        {editingUserRole?.userId === p.userId ? (
                          <div className="flex items-center space-x-2">
                            ...
                            <Button
                              onClick={() => handleUpdateUserRole(p.userId, editingUserRole.currentRole)}
                              size="sm"
              ...
              {eventAnalytics && (
                <div className="mt-6">
                  <h3 className="text-xl font-semibold mb-2">Statistiques de l'événement : {eventAnalytics.eventName}</h3>
                  <p>
                    Temps restant avant le début de l'évènement : {Math.floor(eventAnalytics.timeRemaining / (1000 * 60 * 60))}h{' '}
                    {Math.floor((eventAnalytics.timeRemaining % (1000 * 60 * 60)) / (1000 * 60))}m
                  </p>
                  <div className="grid grid-cols-2 gap-4 text-center my-4">
                    <div className="p-4 bg-blue-100 rounded">
                      <p className="text-2xl font-bold">{eventAnalytics.productsOnSaleCount}</p>
                      <p>Produits en vente</p>
                    </div>
                    <div className="p-4 bg-green-100 rounded">
                      <p className="text-2xl font-bold">{eventAnalytics.productsSoldCount}</p>
                      <p>Produits vendus</p>
                    </div>
                    <div className="p-4 bg-orange-100 rounded">
                      <p className="text-2xl font-bold">${eventAnalytics.totalValueOnSale.toFixed(2)}</p>
                      <p>Valeur totale en vente</p>
                    </div>
                    <div className="p-4 bg-purple-100 rounded">
                      <p className="text-2xl font-bold">${eventAnalytics.totalValueSold.toFixed(2)}</p>
