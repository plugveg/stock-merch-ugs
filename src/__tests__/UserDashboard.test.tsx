...
vi.mock("../components/stock-form", () => ({
  StockForm: ({
    ...
    onSubmit: (data: MockProductFormData) => void;
    onCancel: () => void;
  }) => (
    <div data-testid="stock-form">
      <button onClick={() => onSubmit({ productName: "Test Product" })}>
        Submit Stock Form
      </button>
...
describe("UserDashboard", () => {
  ...
  it("renders the dashboard with user information", () => {
    ...
    expect(
      screen.getByText("Connecté en tant que TestUser"),
    ).toBeInTheDocument();
  ...
  it("shows message when no products exist", () => {
    ...
    expect(
      screen.getByText("Vous n'avez pas encore ajouté de produits."),
    ).toBeInTheDocument();
  });
  ...
  it("opens add product dialog when button is clicked", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /ajouter un produit/i }),
    );
  ...
  it("handles add product error", async () => {
    ...
    await waitFor(() => {
      ...
      expect(global.alert).toHaveBeenCalledWith(
        "Erreur lors de l'ajout du produit : addProduct is not a function",
      );
    });
  ...
  it("displays my events participation", () => {
    ...
    expect(
      screen.getByText("Mes participations aux événements :"),
    ).toBeInTheDocument();
  ...
  it("shows message when no event participation", () => {
    ...
    expect(
      screen.getByText("Vous n'êtes pas encore inscrit pour un événement."),
    ).toBeInTheDocument();
  });
  ...
  it("handles leave event error", async () => {
    ...
    await waitFor(() => {
      expect(consoleErrorSpy).toHaveBeenCalled();
      expect(global.alert).toHaveBeenCalledWith(
        "Error leaving event: Leave event error",
      );
    });
  ...
  it("handles leave event when user is not identified", async () => {
    ...
    (useQuery as Mock)
      ...
      .mockReturnValueOnce(mockMyEvents);
    ...
    expect(global.alert).toHaveBeenCalledWith(
      "Could not identify logged in user.",
    );
    expect(mockRemoveUserFromEvent).not.toHaveBeenCalled();
  });
  ...
  it("prevents form submission when required fields are empty", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /confirmer la disponibilité/i }),
    );
    ...
    await user.click(
      screen.getByRole("button", { name: /confirmer la participation/i }),
    );
