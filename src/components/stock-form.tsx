...
export function StockForm({ initialData, onCancel, onSubmit }: StockFormProps) {
  ...
  const handleArrayChange = (field: 'licenseName' | 'characterName', index: number, value: string) => {
  ...
  const removeArrayField = (field: 'licenseName' | 'characterName', index: number) => {
  ...
  const validateForm = () => {
    ...
    if (formData.licenseName.length === 0 || formData.licenseName.some((item: string) => !item.trim())) {
      newErrors.licenseName = 'Au moins une licence est requise'
    }

    if (formData.characterName.length === 0 || formData.characterName.some((item: string) => !item.trim())) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 px-2">
        ...
        <div className="space-y-2 col-span-1 sm:col-span-2 mb-4">
          ...
          {formData.licenseName.map((value: string | number | readonly string[] | undefined, idx: number) => (
            <div key={idx} className="flex items-center gap-2">
              <Input
                id={`licenseName-${idx}`}
                value={value}
                onChange={(e) => handleArrayChange('licenseName', idx, e.target.value)}
                className="flex-1"
                placeholder="Entrez un nom de licence"
              />
              <Button type="button" variant="outline" onClick={() => removeArrayField('licenseName', idx)}>
                Supprimer
              </Button>
            </div>
          ))}
          <Button type="button" onClick={() => addArrayField('licenseName')} size="sm">
            <Plus className="mr-2 h-4 w-4" />
            Ajouter une licence
          </Button>
          {errors.licenseName && <p className="text-xs text-destructive">{errors.licenseName}</p>}
        </div>

        {/* Multiple character names */}
        <div className="space-y-2 col-span-1 sm:col-span-2 mb-4">
          <Label htmlFor="characterName">Personnage(s)*</Label>
          {formData.characterName.map((value: string | number | readonly string[] | undefined, idx: number) => (
            <div key={idx} className="flex items-center gap-2">
              <Input
                id={`characterName-${idx}`}
                value={value}
                onChange={(e) => handleArrayChange('characterName', idx, e.target.value)}
                className="flex-1"
                placeholder="Entrez un nom de personnage"
              />
              <Button type="button" variant="outline" onClick={() => removeArrayField('characterName', idx)}>
                Supprimer
              </Button>
            </div>
          ))}
          <Button type="button" onClick={() => addArrayField('characterName')} size="sm">
            <Plus className="mr-2 h-4 w-4" />
        ...
        <div className="space-y-2">
          ...
          />
          {errors.purchaseLocation && <p className="text-xs text-destructive">{errors.purchaseLocation}</p>}
        </div>
        ...
        <div className="space-y-2 content-end flex flex-col">
          ...
          <Input
            ...
            onChange={(e) => handleChange('purchasePrice', Number.parseFloat(e.target.value))}
          />
        </div>
        ...
        <div className="space-y-2">
          <Label htmlFor="threshold" className="gap-0 block">
            Seuil de stock bas*
            <span className="ml-1 text-xs text-muted-foreground">(Alertes si le stock est inférieur à ce nombre)</span>
          </Label>
          <Input
            id="threshold"
            type="number"
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleChange('sellPrice', Number.parseFloat(e.target.value))}
              />
            </div>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit">{initialData ? 'Mettre à jour le produit' : 'Ajouter le produit'}</Button>
      </div>
    </form>
  )
}
