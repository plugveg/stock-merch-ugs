...
export default function Products() {
  ...
  const isAdmin = userInConvex?.role === 'Administrator' || userInConvex?.role === 'Board of directors'
  /* Sélecteur d’utilisateur pour l’admin */
  const [selectedUserId, setSelectedUserId] = useState<Doc<'users'>['_id'] | undefined>(isAdmin ? userInConvex?._id : undefined)
  ...
  const lowStockItems = useMemo(
    () => products.filter((item: { quantity: number; threshold: number }) => item.quantity <= item.threshold),
    [products]
  )
        ...
        {isAdmin && (
          <div className="mb-4 max-w-xs flex items-center gap-2">
            ...
            <Select
              ...
              onValueChange={(val) => setSelectedUserId(val === 'all' ? undefined : (val as Doc<'users'>['_id']))}
            >
              <SelectTrigger className="border border-black">
                {selectedUserId === userInConvex?._id
                  ? userInConvex?.nickname
                  : usersLite?.results.find((u) => u._id === selectedUserId)?.label}
                {selectedUserId === undefined ? 'Tous les utilisateurs' : ''}
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Tous les utilisateurs</SelectItem>
                <SelectItem value={userInConvex?._id}>{userInConvex?.nickname}</SelectItem>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger>
                  <Info size={16} className="hover:text-indigo-600 transition-colors" />
                </TooltipTrigger>
                <TooltipContent>
                  <p>L'utilisateur sélectionné sera celui pour lequel vous allez ajouter ou modifier les produits</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
          ...
          <StockForm
            onSubmit={(formData) =>
              addProduct.mutate(
                {
                  ...formData,
                  targetUserId: selectedUserId as Doc<'users'>['_id'] | undefined,
                },
        ...
        >
          <StockForm
            ...
            onSubmit={(data) => {
              ...
              updateProduct.mutate({ id: _id, ...fields }, { onSuccess: () => setEditDialogOpen(false) })
            }}
          ...
          <CardHeader>
            ...
            <CardDescription>
              ...
              {lowStockItems.length > 0 && (
                <span className="text-destructive font-medium">{lowStockItems.length} produits en-dessous du seuil</span>
              )}
            </CardDescription>
          </CardHeader>
          <CardContent>
