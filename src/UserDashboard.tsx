...
export default function UserDashboard() {
  ...
  const setProductAvailability = useMutation(api.functions.products.setProductAvailabilityForEvent)
  const participateInEvent = useMutation(api.functions.products.participateInEvent)
  ...
  const [selectedEventIdForProduct, setSelectedEventIdForProduct] = useState<Id<'events'> | ''>('')
  const [isAvailable, setIsAvailable] = useState(true)

  const [eventToParticipate, setEventToParticipate] = useState<Id<'events'> | ''>('')
    ...
    try {
      ...
      alert(
        `Produit ${isAvailable ? 'marqué disponible' : 'retiré'} pour l'événement ! ${
          isAvailable ? "Les admins peuvent maintenant l'ajouter aux ventes." : ''
        }`
      )
      ...
      <main className="flex-1 p-4 md:p-6 mb-[80px] md:mb-[56px] mt-[140px] md:mt-[56px] space-y-8">
        <section className="p-6 bg-white rounded-lg shadow mx-auto w-3/4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-semibold text-primary">Mes produits</h2>
            ...
            <ResponsiveDialog
              ...
              size="lg"
            >
              <StockForm initialData={undefined} onSubmit={handleAddProduct} onCancel={() => setIsAddModalOpen(false)} />
            </ResponsiveDialog>
          </div>
          <ul className="space-y-2">
            {myProducts.length === 0 && <p>Vous n'avez pas encore ajouté de produits.</p>}
        ...
        <section className="p-6 bg-white rounded-lg shadow mx-auto w-3/4">
          <h2 className="text-2xl font-semibold mb-4 text-primary">Gérer la disponibilité pour un événement</h2>
          <p className="text-sm text-gray-600 mb-2">
            NB : Ceci exprime votre intérêt. Les admins de l'événement ajouteront les produits à la vente définitive.
          </p>
          <form onSubmit={handleSetAvailability} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <select
            ...
            <select
              ...
              onChange={(e) => setSelectedEventIdForProduct(e.target.value as Id<'events'>)}
              className="p-2 border rounded"
              required
            >
              <option value="" disabled>
        ...
        <section className="p-6 bg-white rounded-lg shadow mx-auto w-3/4">
          <h2 className="text-2xl font-semibold mb-4 text-primary">Participer à un événement</h2>
          <form onSubmit={handleParticipate} className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            <select
              value={eventToParticipate}
              onChange={(e) => setEventToParticipate(e.target.value as Id<'events'>)}
              className="p-2 border rounded"
              required
            >
              <option value="" disabled>
          ...
          <div className="mt-4">
            <h3 className="text-lg font-semibold">Mes participations aux événements :</h3>
            {myEventsParticipation.length === 0 && (
              <p className="text-sm text-gray-500">Vous n'êtes pas encore inscrit pour un événement.</p>
            )}
